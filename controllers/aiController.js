...
exports.syncWeeklySpotlight = async (req, res) => {
  try {
    ...
    // Check: Only generate if missing, empty, or older than 7 days
    ...
    //  Hit Gemini 1.5 with dedicated API Key
    const model = "gemini-2.5-flash"; 
    ...
    Return ONLY JSON: { "themeTitle": "string", "themeDescription": "string", "movieTitles": ["Title 1", ...] }`;
    ...
    //  Hit TMDB with Title Cleaning
    const moviePromises = aiParsed.movieTitles.map(async (title) => {
      try {
        // Remove leading numbers (1. ) and trailing years ((2024)) for better search accuracy
        ...
        console.log(`Spotlight Search: "${cleanTitle}" -> Found: ${tmdbRes.data.results.length}`);
        return tmdbRes.data.results[0]; 
      } catch (err) {
        return null;
      }
    ...
    res.status(500).json({ success: false, error: "AI Engine error. Please try again later." });
  }
};